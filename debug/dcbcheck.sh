...
if dcbtool gc ${IFNAME} dcb | grep Status | grep -q Failed ; then
    echo "Unable to communicate with the dcb daemon or DCB capable driver." >&2
    exit 1
fi
...
if dcbtool gc ${IFNAME} pfc | grep Enable | grep -q false ; then
    echo "PFC is not enabled, exectue the following command to turn it on" >&2
...
if dcbtool gc ${IFNAME} app:fcoe | grep Enable | grep -q false ; then
    echo "The FCoE APP TLV is not enabled, exectue the following command to turn it on" >&2
